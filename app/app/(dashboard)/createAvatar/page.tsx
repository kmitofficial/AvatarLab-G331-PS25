...
import { Pause,Play,Plus, Search, List, Info, Camera, Square } from "lucide-react";
...
export default function AvatarsPage() {
  ...
  const [form, setForm] = useState({ name: "", gender: "Male", sentence: "" });
  ...
  const [usersAvatars,setUsersAvatars] = useState<userAvatar[]>();
...
React.useEffect(() => {
  const fetchAvatars = async () => {
    try {
      ...
      // Fetch predefined and user-defined avatars
  ...
  const sentences = [
    "Hello, I'm excited to create my avatar!",
    "This is my digital persona speaking.",
  ...
  const handleNextStep = () => {
    if (currentStep === 1 && form.name.trim() !== "") {
      setCurrentStep(2);
    } else if (currentStep === 2 && recordedBlob && form.sentence) {
      handleSubmit();
    } else {
      alert(
        currentStep === 1
          ...
          : !recordedBlob
          ? "Please record a video before proceeding"
...
const handleSubmit = async () => {
    if (!form.name.trim() || !recordedBlob) {
      alert("Please fill in all required fields and record a video");
      return;
    }
    ...
    if (recordedBlob.size > maxSize) {
      alert("Video file is too large. Please record a shorter video (max 10MB).");
      return;
    }
    ...
    try {
      ...
      setpreDefinedAvatars(predefined || []);
      setuserDefinedAvatars(userdefined || []);
      setUsersAvatars(user || []);

      toast.success(`Your avatar "${form.name}" created successfully!`);
      setRecordedBlob(null);
      setCurrentStep(1);
      setIsDialogOpen(false);
    } catch (error) {
      console.error("Submission error:", error);
      toast.error("There was an error creating your avatar. Please try again.");
    } finally {
  ...
  <DialogContent className="sm:max-w-4xl max-w-[95vw] max-h-[80vh] overflow-y-auto rounded-lg">
    ...
    {currentStep === 1 ? (
      ...
      <div className="grid gap-6 py-4 sm:py-6">
        {/* Video Recording */}
        <div className="relative rounded-md overflow-hidden border border-indigo-200 bg-black aspect-[8/6] max-h-[2000vh] sm:max-h-[40vh]">
          <video
            ref={videoRef}
        ...
        {/* Sentence Selection */}
        <div className="space-y-2">
          <Label htmlFor="sentence" className="text-indigo-700 text-sm sm:text-base">
            Select a Sentence <span className="text-red-500">*</span>
          </Label>
          <Select
            value={form.sentence}
            onValueChange={(value) => setForm((prev) => ({ ...prev, sentence: value }))}
          >
            ...
            <SelectContent>
              {sentences.map((sentence) => (
                <SelectItem key={sentence} value={sentence} className="text-sm sm:text-base">
                  {sentence}
                </SelectItem>
              ))}
            </SelectContent>
          ...
          <p className="text-sm text-gray-500">
            Select a sentence to associate with your avatar
          </p>
        </div>
    ...
    <DialogFooter className="pt-4 sticky bottom-0 bg-white dark:bg-gray-900 pb-8 sm:pb-10">
      ...
      <Button
        ...
        disabled={
          ...
          (currentStep === 2 && (!recordedBlob || !form.sentence))
        }
        className="bg-indigo-600 hover:bg-indigo-700 text-white text-sm sm:text-base px-3 sm:px-4 py-2"
      >
        {isSubmitting ? (
